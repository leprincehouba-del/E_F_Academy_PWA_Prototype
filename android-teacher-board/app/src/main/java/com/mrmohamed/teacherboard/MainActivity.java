...
public final class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        pageStatus=new TextView(this); pageStatus.setText("v22"); pageStatus.setTextColor(Color.WHITE); pageStatus.setTextSize(13f); pageStatus.setGravity(Gravity.CENTER); toolbar.addView(open); toolbar.addView(fit); toolbar.addView(zoomOut); toolbar.addView(zoomIn); toolbar.addView(hand);
        ...
        open.setOnClickListener(v->openPdfPicker()); fit.setOnClickListener(v->setZoomImmediately(pdfView.getMinZoom())); hand.setOnClickListener(v->{drawMode=false;drawingView.setInputEnabled(false);drawingView.setVisibility(View.VISIBLE);pdfView.bringToFront();drawingView.bringToFront();drawingView.setClickable(false);});
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){ super.onActivityResult(requestCode,resultCode,data); if(requestCode!=OPEN_PDF_REQUEST||resultCode!=RESULT_OK||data==null)return; Uri uri=data.getData(); if(uri==null)return; try{getContentResolver().takePersistableUriPermission(uri,Intent.FLAG_GRANT_READ_URI_PERMISSION);}catch(SecurityException ignored){}
        pageStatus.setText("v22");
        pdfView.fromUri(uri).enableSwipe(true).swipeHorizontal(false).enableDoubletap(true).defaultPage(0)
          .onLoad(n->{pageStatus.setText("v22");drawingView.setPage(0);}).onPageChange((p,n)->{pageStatus.setText("v22");drawingView.setPage(p);})
