...
public final class DrawingView extends View {
  ...
  public void setLiveTarget(View v){ liveTarget=v; }
  ...
  @Override public boolean onTouchEvent(MotionEvent e){
    ...
    switch(e.getActionMasked()){
      ...
      case MotionEvent.ACTION_MOVE: if(active!=null){ active.lineTo(x,y); if(liveCanvas!=null){ liveBitmap.eraseColor(Color.TRANSPARENT); liveCanvas.drawPath(active,activePaint); } float pad=(tool==Tool.PEN?penWidth:eraserWidth)+6f; int l=(int)(Math.min(lastX,x)-pad), t=(int)(Math.min(lastY,y)-pad), r=(int)(Math.max(lastX,x)+pad), b=(int)(Math.max(lastY,y)+pad); lastX=x; lastY=y; if(liveTarget!=null)liveTarget.invalidate(l,t,r,b); else invalidate(l,t,r,b); } return true;
